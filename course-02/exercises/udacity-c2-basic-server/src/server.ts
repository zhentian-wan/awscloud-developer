...
import { Car, cars as cars_list, getCarById, addCar } from "./cars";
...
(async () => {
  ...
  app.get("/cars/:id", (req: Request, res: Response) => {
    ...
    if (id == undefined) {
      return res.status(400).send(`Car id is not valid`);
    }
    const car = getCarById(id);
    if (!car) {
      return res.status(404).send(`Car is not found by id: ${id}`);
    }
